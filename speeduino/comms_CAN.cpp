...
#if defined(NATIVE_CANFD_AVAILABLE)
CANFD_message_t outFDMsg;
CANFD_message_t inFDMsg;
#endif
...
void sendDCTPowertrain() {
    PowertrainMessage(PT_CAN_THRTL);
}

void PowertrainMessage(uint16_t PowertrainMessageID) {
  switch (PowertrainMessageID) {
    case PT_CAN_THRTL:
        uint8_t temp_TPS;
        uint32_t temp_RPM;
        temp_TPS = map(currentStatus.TPS, 0, 200, 1, 254);
        temp_RPM = currentStatus.RPM * 4;
        outFDMsg.id = PowertrainMessageID;
        outFDMsg.len = 8;
        outFDMsg.buf[0] = 0x5F;
        outFDMsg.buf[1] = 0x59;
        outFDMsg.buf[2] = temp_TPS; // TPS sensor 1
        outFDMsg.buf[3] = 254 - temp_TPS; // TPS sensor 2
        outFDMsg.buf[4] = lowByte(uint16_t(temp_RPM));
        outFDMsg.buf[5] = highByte(uint16_t(temp_RPM));
        outFDMsg.buf[6] = 0x80;
        outFDMsg.buf[7] = 0x99;
    }
}
...
void DashMessage(uint16_t DashMessageID) {
    ...
    case CAN_BMW_DME2:
        ...
        if (temp_CLT > 255) {
            temp_CLT = 255;
        } // CLT conversion can yield to higher values than what fits to byte, so limit the maximum value to 255.
